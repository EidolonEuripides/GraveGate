...
const dungeonMaskPalettes = require("../../data/dungeon/dungeon-mask-palettes.json");
...
function loadDungeonMaskPalette(options) {
  ...
  if (!palette) {
    throw new Error(`unknown dungeon mask palette: ${paletteId}`);
  }
  ...
  return {
    ...
    entries: (palette.entries || []).map((entry) => ({
      ...
      label: entry.label || entry.marker_type || "",
      marker_type: entry.marker_type || "open",
...
function getMaskMetrics(map, bitmap) {
...
function sampleTileMarker(bitmap, metrics, tile, palette, tolerance) {
  ...
  samplePoints.forEach((point) => {
    ...
    const key = `${match.hex}:${match.marker_type}`;
    counts.set(key, {
...
function inferExitDirection(position, grid) {
  if (position.y <= 0) return "north";
  if (position.y >= grid.height - 1) return "south";
  if (position.x <= 0) return "west";
  if (position.x >= grid.width - 1) return "east";
  return "exit";
}

function buildDungeonEntriesFromMaskBitmap(map, bitmap, options) {
  const palette = loadDungeonMaskPalette({
    palette_id: options && options.palette_id
      ? options.palette_id
      : (map.asset && map.asset.dungeon_mask_palette_id) || "mspaint_dungeon_markers",
    palette_path: options && options.palette_path
      ? options.palette_path
      : (map.asset && map.asset.dungeon_mask_palette_path) || ""
  });
  const tolerance = Number.isFinite(options && options.color_tolerance)
    ? Number(options.color_tolerance)
    : (map.asset && Number.isFinite(map.asset.dungeon_mask_color_tolerance)
      ? Number(map.asset.dungeon_mask_color_tolerance)
      : 24);
  const metrics = getMaskMetrics(map, bitmap);
  const counts = {};
  const enemies = [];
  const traps = [];
  const exits = [];
  ...
  for (let y = 0; y < map.grid.height; y += 1) {
    for (let x = 0; x < map.grid.width; x += 1) {
      const entry = sampleTileMarker(bitmap, metrics, { x, y }, palette, tolerance);
      if (!entry) {
        continue;
      }

      counts[entry.marker_type] = (counts[entry.marker_type] || 0) + 1;
      const position = { x, y };

      if (entry.marker_type === "party") {
        partyPosition = position;
        continue;
      }

      if (entry.marker_type === "enemy") {
        enemies.push({
          token_id: `mask-enemy-${x}-${y}`,
          actor_id: `mask-enemy-${x}-${y}`,
          label: "Enemy",
          badge_text: `E${enemies.length + 1}`,
          position
        });
        continue;
      }

      if (entry.marker_type === "trap") {
        traps.push({
          object_id: `mask-trap-${x}-${y}`,
          object_type: "trap",
          name: "Trap",
          position,
          state: {}
      ...
      if (entry.marker_type === "exit") {
        exits.push({
          direction: inferExitDirection(position, map.grid),
          to_room_id: `mask-exit-${x}-${y}`,
          position,
          label: "Exit"
        });
      }
  ...
  return {
    ...
    visible_enemy_tokens: enemies,
    objects: traps,
    exits,
    encounter_triggers: enemies.map((entry) => ({
      encounter_id: `${entry.token_id}-encounter`,
      position: entry.position,
      label: entry.badge_text
    })),
    summary: {
      palette_id: palette.palette_id,
      tile_count: map.grid.width * map.grid.height,
      marker_type_counts: counts
    }
  };
}

async function buildDungeonEntriesFromMaskPath(map, options) {
  const requestedPath = options && options.mask_path
    ? options.mask_path
    : (map.asset && map.asset.dungeon_mask_path) || "";
  if (!requestedPath) {
    return {
      party_position: null,
  ...
  if (!fs.existsSync(absoluteMaskPath)) {
    throw new Error(`dungeon mask not found: ${requestedPath}`);
  }

  const image = await Jimp.read(absoluteMaskPath);
  const built = buildDungeonEntriesFromMaskBitmap(map, image.bitmap, options);
  return {
    ...built,
